...
tools:
  ...
  - mcp_roblox_get_connection_status
...
2. **Start play** — `start_stop_play` mode `start_play`. Wait ~2 seconds for character to spawn.
3. **Initial screenshot** — `capture_studio_window` to see the starting state + record window dimensions.
4. **Test the feature** — sequence of `simulate_input` actions appropriate to what's being tested:
   - Movement: `key_hold` W/A/S/D for a few seconds
   - Jump: `key_press` Space
   - Sprint: `key_hold` LeftShift
   - Click UI: `mouse_click` at coords from the screenshot
   - Camera/aim: `mouse_move` or `mouse_drag`
5. **Observe** — `capture_studio_window` after each meaningful action.
6. **Read logs** — `get_console_output` (use `channels: ["Error", "Warning"]` + `since_seconds: 30` to focus on recent issues).
7. **Stop play** — `start_stop_play` mode `stop`.
8. **Report** — what works, what doesn't, screenshots referenced.
