...
tools:
  ...
  - mcp__blender__blender_execute_python
...
`import_blender_model`. For advanced operations (UV unwrap, image textures,
procedural generation, node graphs), use `blender_execute_python` to run
arbitrary Blender Python code. Always confirm `blender_get_connection_status`
returns `addonConnected:true` before building. Keep meshes under the export
